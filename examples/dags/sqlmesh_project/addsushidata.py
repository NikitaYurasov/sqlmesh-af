...
def add_items_data(
    ...
    for dt in iter_dates(start, end):
        ...
        if to_ds(dt) not in existing_item_ds:
...
def add_order_items_data(
    ...
    db_con.sql(f'CREATE TABLE IF NOT EXISTS {out_tbl} (id int, order_id int, item_id int, quantity int, ds date)')
    ...
    for dt in iter_dates(start, end):
        ...
        orders_to_process = set(orders['id'].tolist()) - set(existing_order_items['order_id'].tolist())
        ...
        dt_df = pd.concat(order_dfs, ignore_index=True).reset_index().rename(columns={'index': 'id'})
...
def create_customer_data(
    ...
    if marketing_tbl not in tables:
        marketing_df = (
            pd.DataFrame(
                {
                    'name': customers,
                    'status': random.choices(['ACTIVE', 'INACTIVE'], weights=[80, 20], k=len(customers)),
                }
            )
            .reset_index()
            .rename(columns={'index': 'id'})
        ...
        db_con.sql(f'CREATE TABLE IF NOT EXISTS {raw_schema}.{marketing_tbl} (id int, customer_id int, status text)')
        db_con.sql(f'INSERT INTO {raw_schema}.{marketing_tbl} SELECT * FROM marketing_df')

    if demographics_tbl not in tables:
        demographics_df = (
            pd.DataFrame(
                {
                    'name': customers,
                    'status': random.sample(
                        [str(zip) for zip in range(10000, 11000)],
                        k=len(customers),
                    ),
                }
            )
        ...
        db_con.sql(f'CREATE TABLE IF NOT EXISTS {raw_schema}.{demographics_tbl} (id int, customer_id int, zip text)')
